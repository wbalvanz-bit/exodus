...
export default function ExodusCreator() {
  ...
  // PRODIGY: Dynamically calculate Neural Overload Cap (INT Mod + Prof Bonus)
  const calculatedOverloadCap = Math.max(1, modifiers.int + profBonus);

  // Auto-Update Mech Stats based on Chassis & Level changes
  useEffect(() => {
        ...
        <div className="w-full lg:w-7/12 p-6 md:p-10 lg:border-r border-cyan-500/30 flex flex-col gap-6 relative">
          ...
          {activeTab === "tech" && (
            <div className="flex flex-col gap-8 animate-in fade-in duration-300 h-full overflow-y-auto pr-2 pb-4">
              ...
              {/* REMNANT BLOCK (Hidden for Prodigy as it moves to their class tab) */}
              {charClass !== 'prodigy' && (
                <div className="space-y-4">
                  <div className="flex justify-between items-end border-b border-slate-700 pb-2">
